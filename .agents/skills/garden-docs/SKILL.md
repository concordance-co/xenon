...
- Active-project `README.md`, `PHASE.md`, `REAL_DATA.md`,
...
- Old projects flagged in `AGENTS.md` as predating current methodology
  (`MOREBENCH/`, `COUNSELBENCH/`, `DX_TERMINAL/`) — only touch if user
  explicitly asks
...
1. **Dead references.** Every relative link, file path, function name, module
   ...
   - For symbols: `grep -rn "<symbol>" pipelines_v2/ scripts/ dashboard/` — if
...
4. **Bloat / redundancy.** Sections covered better elsewhere; trailing
   ...
   Y") — the user has flagged this twice and does not want it.
...
Output keyed by file. Example shape:
    ...
    AGENTS.md
      ...
      L131: claim "MOREBENCH/COUNSELBENCH/DX_TERMINAL predate current
        methodology" — DX_TERMINAL has phase_15/phase_16 actively running on
        current platform; either narrow the claim or drop DX_TERMINAL from it
...
Per user preference: discuss before compiling. Walk the punch list section by
section with the user. Apply edits surgically. Do NOT batch a sweep of edits
followed by "here's everything I changed" — that loses the conversation.

After applied edits, run `git log -1 --stat` to verify the diff if a commit
was made (per user preference: Edit tool writes working tree only; verify
commits actually contain the claimed diff).
...
- Adding new top-level docs. Per user preference, fold new content into
  existing methodology/operations/platform docs instead.
- Using "X, not Y" foil framing in any rewrites you propose.
- Touching old projects (`MOREBENCH/`, `COUNSELBENCH/`, `DX_TERMINAL/`)
  unprompted.
