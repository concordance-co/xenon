...
How a research thread moves from question to finding. This is the methodology spine. Every workspace and phase lives inside this model.

## Structure

Work is organized in three levels:

- **Workspace** — one dataset and one system under study. The real-data context lives here.
- **Research thread** — one question about that system. Owns its synthetic seed, probes, findings.
- **Phase** — a chapter of work inside a research thread. A new phase begins when a non-negligible change commits the work to a new direction.

The flywheel describes the stages a research thread can move through. Stages vary in size, threads visit different subsets of them, and the loopbacks between stages are where the research happens.
...
- `workspaces/<name>/REAL_DATA.md` — living real-data context doc (stage-1 artifact). See `templates/REAL_DATA.md`.
- `workspaces/<name>/shared/` — workspace-local helpers (`paths.py`, `neon.py`, etc.)
- `workspaces/<name>/scripts/` — workspace-level builders
- `workspaces/<name>/<thread>/phase_XX/` — same shape, when a workspace has multiple distinct research questions
- `workspaces/<name>/benchmark_context.md` — frozen sidecar for benchmark work
...
3. Scaffold the workspace per the layout above. The first phase is `phase_00` — the framing pass: orientation, source scan, shape decisions. Hypothesis sketches and substrate analysis live here. Capture and synth design come later.
4. Report back with what the substrate looks like, what scaffold went up, and what framings the data invites.

The user takes it into hypothesis mode from there. Phase 0 closes when the work has a defensible first research question and the next phase has its premise.
...
Artifact: the real-data context doc for this workspace, versioned and dated. Living document — revisit across the research thread's lifetime. See `templates/REAL_DATA.md`.
