...
A hypothesis catalog is a research-thread-level cross-phase reference that
collects candidate research directions worth running through the flywheel
but not yet committed to a phase. One per research thread, append-only
with explicit closure semantics.

This doc explains the why and how. The instance lives at
`workspaces/<workspace>/[<thread>/]HYPOTHESES.md` — at the research-thread
level when there is one, otherwise at the workspace root.
...
- Hypotheses whose evidence would require a different model, dataset, or
  research thread. Those belong to that other thread's catalog, not this
  one.
...
- Speculation about general questions in the field. The catalog is
  scoped to *this research thread's* questions.
...
real failure mode in long-running research threads.
...
- `HYPOTHESES.md` (per workspace or research thread) — the candidates. Tells
...
honest output of the research thread.
...
- The research thread has at least two closed phases with non-trivial open
  threads.
...
Use this file as the schema reference for new catalogs.
