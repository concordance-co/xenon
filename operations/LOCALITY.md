...
- **Home.** Neon table. Uploaded via repeatable uploader scripts at the project level.
- **Read.** Same as synth datasets.
- **Local.** Same rules. Uploader scripts live at `projects/<p>/<sub>/phase_NN/scripts/upload_*.py`; they are the only sanctioned path for getting real data into Neon.
...
- **Home.** Local, under `projects/.../reports/`.
- **Read.** Locally.
