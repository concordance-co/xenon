...
Report artifacts live in the workflow workspace, typically at:

- `reports/`
- `workflows/<workflow_name>/reports/`
...
- **PNG and PDF outputs in the `reports/` tree only.** Don't scatter chart outputs across scratch dirs or workflow roots.
- **Use `paths.py` helpers for locations.** Don't hardcode workspace-specific paths in report scripts.
...
- **Typst** for polished deliverables — phase exit reports meant to persist. Commit the `.typ` source and the generated PDF.
...
uv run python workflows/<workflow_name>/reports/scripts/build_charts.py

# compile Typst to PDF
typst compile workflows/<workflow_name>/reports/report.typ
```
