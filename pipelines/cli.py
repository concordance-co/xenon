...
def _run_prepare_passthrough(argv: list[str]) -> int:
    from pipelines.interp.prepare import main

    return int(main(argv) or 0)


def _run_capture_passthrough(argv: list[str]) -> int:
    from pipelines.interp.capture import main
...
def _run_capture(argv: list[str]) -> int:
    ...
    from pipelines.interp.capture import CaptureConfig, run_capture
    ...
    with _open_conn() as conn:
        ...
        try:
            ...
            finish_workflow_run(conn, run_id=run["id"], status="failed", error_text=str(exc))
            raise
    _print_json({"run_id": run["id"], "activations_dir": str(ns.output_dir), "publication": dataset_name})
    return 0
