...
interface Props {
  command?: string
  jobId?: string  // reconnect to existing job
  minimized: boolean
  onMinimize: () => void
  onRestore: () => void
  onClose: () => void
  onJobId?: (id: string) => void  // called when we get a job_id from the server
}
...
export function CommandRunner({ command, jobId, minimized, onMinimize, onRestore, onClose, onJobId }: Props) {
  const [lines, setLines] = useState<LogLine[]>([])
  const [running, setRunning] = useState(true)
  const [returnCode, setReturnCode] = useState<number | null>(null)
  const [displayCmd, setDisplayCmd] = useState(command ?? '')
  ...
  const startStream = useCallback(async () => {
    ...
    // Decide whether to start a new command or reconnect
    ...
    try {
      ...
      if (running) {
        setRunning(false)
        if (returnCode === null) setReturnCode(-1)
      }
    } catch (err) {
  ...
  }, [command, jobId, onJobId]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  if (minimized) {
    return (
      <button className={styles.pill} onClick={onRestore}>
        ...
        <span>{running ? 'Running...' : returnCode === 0 ? 'Done' : 'Failed'}</span>
  ...
  return (
    <div className={styles.overlay} onClick={e => { if (e.target === e.currentTarget) onMinimize() }}>
      <div className={styles.panel}>
        <div className={styles.head}>
          <div className={styles.headLeft}>
            ...
            <span className={styles.headTitle}>
              {running ? 'Running...' : returnCode === 0 ? 'Complete' : 'Failed'}
            </span>
          </div>
        ...
        <div className={styles.cmd}>$ {displayCmd}</div>
        <pre className={styles.log} ref={logRef}>
          {lines.length === 0 && running && 'Waiting for output...\n'}
          {lines.map((line, i) => (
