"""Low-level vLLM hooks for Qwen3 MoE router capture.

This module patches Qwen3 MoE block forwards so the vLLM capture engine can
record router logits during prefill. It is internal plumbing used by
``pipelines.interp.modal_vllm_engine`` rather than a user-facing entrypoint.
"""
