"""Worker-side activation-patching utilities for vLLM Qwen models.

This module follows the same pattern as :mod:`pipelines.interp.modal_qwen_moe_hooks`:
it installs wrapped forwards inside vLLM workers in eager mode and applies
tokenwise interventions to a caller-specified token span.
"""
...
class _ActivationPatchedLayer:  # concrete base swapped at runtime when torch is available
    pass
...
def _selected_component_rows(basis_layer: dict[str, Any], spec: ActivationPatchSpec, layer_idx: int) -> Any:
...
def _patch_hidden_states_for_layer(
    ...
    *,
    ...
    if patched is None:
        patched, stats = apply_activation_patch_tensor(
            hidden_states,
            mean=mean,
            scale=scale,
...
def _ensure_batch_tensor_stats_buffers(
    ...
    *,
    ...
    buffers = getattr(model, "_activation_patch_batch_tensor_stats", None)
    if not isinstance(buffers, dict):
        buffers = {}
        model._activation_patch_batch_tensor_stats = buffers
...
def _ensure_batch_runtime_state_buffers(
    ...
    *,
    ...
    buffers = getattr(model, "_activation_patch_batch_runtime_state", None)
    if not isinstance(buffers, dict):
        buffers = {}
        model._activation_patch_batch_runtime_state = buffers
    layer_buffers = buffers.get(int(layer_idx))
    if (
        not isinstance(layer_buffers, dict)
...
def _load_batch_runtime_state(model: Any, batch_specs: list[dict[str, Any]]) -> None:
    ...
    basis = getattr(model, "_activation_patch_basis", None)
    ...
    for slot_idx, batch_spec in enumerate(batch_specs[:_MAX_BATCH_PATCH_STATS_SLOTS]):
        ...
        if not isinstance(payload, dict):
            continue
        spec = ActivationPatchSpec(**payload)
        ...
        for layer_idx in spec.target_layers:
            ...
            runtime_buffers = model._activation_patch_batch_runtime_state.get(int(layer_idx))
...
def _record_batch_tensor_stats(
    ...
    *,
    ...
    if slot_idx < 0 or slot_idx >= _MAX_BATCH_PATCH_STATS_SLOTS:
        return
    selected_rows = _selected_component_rows(basis_layer, spec, layer_idx)
    tensors = summarize_activation_patch_tensor_tensors(
...
def harvest_batch_tensor_stats(model: Any, batch_specs: list[dict[str, Any]]) -> None:
    ...
    if not isinstance(buffers, dict) or not batch_specs:
        return
    stats_by_req = getattr(model, "_activation_patch_last_stats_by_req", None)
    if not isinstance(stats_by_req, dict):
        stats_by_req = {}
        model._activation_patch_last_stats_by_req = stats_by_req
    coverage_by_req = getattr(model, "_activation_patch_coverage_by_req", None)
    if not isinstance(coverage_by_req, dict):
        coverage_by_req = {}
        model._activation_patch_coverage_by_req = coverage_by_req
...
def _apply_layer_output_patching(
    *,
    ...
    force_custom_op_presence = bool(
        getattr(owner_model, "_activation_patch_force_custom_op_presence", False)
    )

    if force_custom_op_presence:
        basis = getattr(owner_model, "_activation_patch_basis", None)
        hidden = _extract_hidden_tensor(output)
        if hidden is None:
            return output
        if not isinstance(basis, dict) or layer_idx not in basis:
            return output
        runtime_state = getattr(owner_model, "_activation_patch_batch_runtime_state", None)
        layer_state = runtime_state.get(int(layer_idx)) if isinstance(runtime_state, dict) else None
        if not isinstance(layer_state, dict):
            return output
        layer_buffers = getattr(owner_model, "_activation_patch_batch_tensor_stats", {}).get(int(layer_idx))
        if not isinstance(layer_buffers, dict):
            return output
        if custom_op is None:
            return output
        compiled_modes = getattr(owner_model, "_activation_patch_compiled_batch_modes", {})
    ...
    batch_specs = getattr(owner_model, "_activation_patch_batch_specs", None)
    ...
    spec = getattr(owner_model, "_activation_patch_spec", None)
    if spec is None or layer_idx not in spec.target_layers:
        ...
        return output
    applied_layers = getattr(owner_model, "_activation_patch_applied_layers", None)
    ...
    hidden = _extract_hidden_tensor(output)
    if hidden is None:
        patch_stats = getattr(owner_model, "_activation_patch_last_stats", None)
        if not isinstance(patch_stats, dict):
            patch_stats = {}
            owner_model._activation_patch_last_stats = patch_stats
    ...
    if stats is not None:
        ...
        if stats.get("status") != "skipped":
            ...
            if not isinstance(applied_layers, set):
                applied_layers = set()
                owner_model._activation_patch_applied_layers = applied_layers
...
try:
    _ActivationPatchedLayer = _build_activation_patched_layer_class()
except Exception:
    _ActivationPatchedLayer = _ActivationPatchedLayer


def install_qwen3_moe_activation_patch_hooks() -> bool:
    try:
    ...
    if getattr(Qwen3MoeDecoderLayer, "_activation_patch_forward_installed", False):
        return True
    ...
    def patched_model_init(self: Any, *args: Any, **kwargs: Any) -> None:
        ...
        for layer_idx, layer in find_decoder_layers(self).items():
            ...
            layer.activation_patch_layer_idx = int(layer_idx)
            layer._activation_patch_owner_ref = weakref.ref(self)
            layer._activation_patch_class_hooked = True
        self._activation_patch_class_hooked = True

    def patched_layer_forward(self: Any, *args: Any, **kwargs: Any) -> Any:
        output = original_layer_forward(self, *args, **kwargs)
        owner_ref = getattr(self, "_activation_patch_owner_ref", None)
        owner_model = owner_ref() if owner_ref is not None else None
        layer_idx = getattr(self, "activation_patch_layer_idx", None)
    ...
    Qwen3MoeForCausalLM._activation_patch_init_installed = True
    Qwen3MoeDecoderLayer._activation_patch_forward_installed = True
    Qwen3MoeForCausalLM._activation_patch_original_init = original_model_init
    Qwen3MoeDecoderLayer._activation_patch_original_forward = original_layer_forward
    return True
...
def init_activation_patching(model: Any) -> None:
    ...
    if getattr(model, "_activation_patch_initialized", False):
        return
    ...
    early_hooked = all(
        getattr(layer, "_activation_patch_class_hooked", False) for layer in layers.values()
    )
    if not early_hooked:
        for layer_idx, layer in layers.items():
            if isinstance(layer, _ActivationPatchedLayer):
                continue
            wrapped = _ActivationPatchedLayer(model, layer_idx, layer)
    ...
    model._activation_patch_initialized = True
    model._activation_patch_basis = placeholder_basis
    model._activation_patch_spec = None
    model._activation_patch_last_stats = {}
    model._activation_patch_applied_layers = set()
    model._activation_patch_batch_specs = []
    model._activation_patch_last_stats_by_req = {}
    model._activation_patch_coverage_by_req = {}
    model._activation_patch_batch_tensor_stats = {}
    model._activation_patch_batch_runtime_state = {}
    model._activation_patch_compiled_batch_modes = {}
    model._activation_patch_force_custom_op_presence = False
...
def register_patch_basis(model: Any, basis_payload: dict[int, dict[str, Any]]) -> dict[str, Any]:
    ...
    if not getattr(model, "_activation_patch_initialized", False):
        init_activation_patching(model)

    device = _infer_model_device(model)
    current_basis = getattr(model, "_activation_patch_basis", None)
    ...
    model._activation_patch_basis = registered
...
def set_patch_spec(model: Any, patch_spec: ActivationPatchSpec | dict[str, Any]) -> dict[str, Any]:
    spec = patch_spec if isinstance(patch_spec, ActivationPatchSpec) else ActivationPatchSpec(**patch_spec)
    model._activation_patch_spec = spec
    model._activation_patch_last_stats = {}
    model._activation_patch_applied_layers = set()
    return {
        "mode": spec.mode,
...
def set_batch_patch_specs(model: Any, batch_specs: list[dict[str, Any]]) -> None:
    model._activation_patch_batch_specs = [dict(spec) for spec in batch_specs]
    compiled_modes: dict[int, str] = {}
    enforce_compiled_mode = bool(getattr(model, "_activation_patch_force_custom_op_presence", False))
    for batch_spec in model._activation_patch_batch_specs:
        payload = batch_spec.get("patch_spec")
        if not isinstance(payload, dict):
            continue
        spec = ActivationPatchSpec(**payload)
    ...
    model._activation_patch_compiled_batch_modes = compiled_modes
    _reset_batch_tensor_stats(model)
    _reset_batch_runtime_state(model)
    if getattr(model, "_activation_patch_force_custom_op_presence", False):
        _load_batch_runtime_state(model, model._activation_patch_batch_specs)
    if not isinstance(getattr(model, "_activation_patch_last_stats_by_req", None), dict):
        model._activation_patch_last_stats_by_req = {}
    if not isinstance(getattr(model, "_activation_patch_coverage_by_req", None), dict):
        model._activation_patch_coverage_by_req = {}
...
def collect_patch_stats(model: Any, req_id: str | None = None) -> dict[Any, dict[str, Any]]:
    if req_id is not None:
        stats_by_req = getattr(model, "_activation_patch_last_stats_by_req", {})
        req_key = str(req_id)
    ...
    stats = getattr(model, "_activation_patch_last_stats", {})
    return {
        int(layer): dict(payload)
...
def restore_original_forwards(model: Any) -> None:
    ...
    if container is not None:
        for idx, layer in list(enumerate(container)):
            original = getattr(layer, "_activation_patch_original_layer", None)
            if original is not None:
                container[idx] = original
    for attr in (
        "_activation_patch_initialized",
        "_activation_patch_basis",
        "_activation_patch_spec",
        "_activation_patch_last_stats",
        "_activation_patch_applied_layers",
        "_activation_patch_batch_specs",
        "_activation_patch_last_stats_by_req",
        "_activation_patch_coverage_by_req",
        "_activation_patch_batch_tensor_stats",
        "_activation_patch_batch_runtime_state",
        "_activation_patch_force_custom_op_presence",
    ):
        if hasattr(model, attr):
            delattr(model, attr)


def teardown_activation_patching(model: Any) -> None:
