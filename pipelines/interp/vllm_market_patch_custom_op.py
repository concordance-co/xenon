...
def summarize_market_patch_tensor(
...
def summarize_market_patch_tensor_tensors(
    original_hidden: torch.Tensor,
    patched_hidden: torch.Tensor,
    *,
    mean: torch.Tensor,
...
def _register_torch_library_project_out_op() -> None:
    ...
    @custom_op("xenon_market_patch::project_out", mutates_args=())
    def _project_out_op(
        hidden_states: torch.Tensor,
        mean: torch.Tensor,
        scale: torch.Tensor,
        ...
        patched, _ = apply_market_patch_tensor(
            hidden_states,
            mean=mean,
            scale=scale,
            safe_scale=safe_scale,
...
def _register_torch_library_project_out_batch_op() -> None:
    ...
    @custom_op(
        "xenon_market_patch::project_out_batch",
...
def _register_torch_library_swap_components_batch_op() -> None:
    ...
    @custom_op(
        "xenon_market_patch::swap_components_batch",
        mutates_args={"valid", "scalars", "coeff_before", "coeff_after"},
    )
...
if _VLLMCustomOp is not None:
    ...
    @_VLLMCustomOp.register("market_patch_hidden_states")
    class MarketPatchHiddenStatesOp(_VLLMCustomOp):
        ...
        def forward_cuda(self, *args: Any, **kwargs: Any) -> torch.Tensor:
            ...
            if (
                ...
                and hasattr(torch.ops, "xenon_market_patch")
                and hasattr(torch.ops.xenon_market_patch, "project_out_batch")
            ):
                return torch.ops.xenon_market_patch.project_out_batch(
                    hidden_states,
                    mean,
                    scale,
                    safe_scale,
            ...
            if (
                ...
                and random_rows is None
                and hasattr(torch.ops, "xenon_market_patch")
                and hasattr(torch.ops.xenon_market_patch, "swap_components_batch")
            ):
                return torch.ops.xenon_market_patch.swap_components_batch(
                    hidden_states,
                    mean,
                    scale,
                    safe_scale,
            ...
            if (
                ...
                and random_rows is None
                and hasattr(torch.ops, "xenon_market_patch")
                and hasattr(torch.ops.xenon_market_patch, "project_out")
            ):
                return torch.ops.xenon_market_patch.project_out(
                    hidden_states,
                    mean,
                    scale,
                    safe_scale,
    ...
    class MarketPatchHiddenStatesOp:  # pragma: no cover - exercised only when vLLM is installed.
        pass


def build_market_patch_hidden_states_op() -> Any | None:
    if _VLLMCustomOp is None:
        return None
    return MarketPatchHiddenStatesOp()
