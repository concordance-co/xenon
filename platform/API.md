...
Policy note:
- For real, shared, or durable project datasets, prefer
  `Dataset.from_postgres(...)` with `PostgresSource.from_env("XENON_NEON_DATABASE_URL")`.
- File-backed constructors such as `from_json(...)` and `from_parquet(...)`
  are intended for local smoke fixtures, imports, audit exports, and scratch
  artifacts, not as the canonical source for shared project datasets.
...
Constructors/methods:
- `PostgresSource.from_env("XENON_NEON_DATABASE_URL")`
...
- for Modal-backed workflows, prefer explicit narrow roots such as
  `("pipelines_v2", "scripts")` or one workspace-local package root
...
Typical use:
- workspace-specific behavioral relabeling
...
runner = ModalRunner(
    resources=ModalResources(
        gpu="A100-80GB",
        secrets=(ModalSecret.from_env_var("XENON_NEON_DATABASE_URL", secret_name="xenon-db"),),
    ),
...
- catalog aliases and spec snapshots described in `ARCH` are not implemented yet.
