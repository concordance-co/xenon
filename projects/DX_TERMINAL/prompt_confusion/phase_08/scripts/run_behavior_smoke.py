...
APP_NAME = "xenon-prompt-confusion-phase8-behavior-smoke"
MODEL_ID = "Qwen/Qwen3-30B-A3B"
DEFAULT_INPUT = Path(
    "projects/DX_TERMINAL/prompt_confusion/phase_08/outputs/phase_08_dataset/phase_08_dataset.jsonl"
)
DEFAULT_OUTPUT = Path(
    "projects/DX_TERMINAL/prompt_confusion/phase_08/reports/behavior_smoke.json"
)
DEFAULT_OUTPUT_THINKING = Path(
    "projects/DX_TERMINAL/prompt_confusion/phase_08/reports/behavior_smoke_thinking.json"
)
...
def generate_rows(
    ...
    *,
    ...
    cfg = VLLMCaptureConfig(
        output_dir=Path("/tmp/phase08_behavior_smoke"),
...
def _summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        key = f"{row['target_dimension']}::{row['strategy_direction']}::{row['conflict_band']}"
...
def main(
    ...
    generations = generate_rows.remote(
        sampled,
