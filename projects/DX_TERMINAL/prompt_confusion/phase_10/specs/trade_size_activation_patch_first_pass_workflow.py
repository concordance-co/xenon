...
MODAL_ARTIFACT_ROOT = "/data/artifacts/prompt_confusion_trade_size_activation_patch_first_pass"
LOCAL_ARTIFACT_ROOT = Path("artifacts") / "prompt_confusion_trade_size_activation_patch_first_pass"
WRITE_LAYER = 32
...
def build_dataset() -> Dataset:
    return Dataset.from_records(
        ...
        name="prompt_confusion_trade_size_activation_patch_first_pass",
    )
...
def build_workflow(dataset: Dataset | None = None) -> WorkflowSpec:
    ...
    return WorkflowSpec(
        ...
        steps=(
            ...
            WorkflowStep(
                name="trade_size_centroids_aligned_patch",
                runner="cpu",
            ...
            WorkflowStep(
                name="trade_size_centroids_same_label_control",
                runner="cpu",
                depends_on=("capture_prompt_eos_residual",),
                spec=CentroidSpec(
                    feature=StepRef("capture_prompt_eos_residual").feature("residual_prompt_eos"),
            ...
            WorkflowStep(
                ...
                runner="gpu",
                depends_on=(
                    "trade_size_centroids_aligned_patch",
                    "trade_size_centroids_same_label_control",
                    "trade_size_subspace",
                ),
            ...
            WorkflowStep(
                name="swap_to_aligned_centroid",
                runner="gpu",
                depends_on=("baseline_conflict_rows", "trade_size_centroids_aligned_patch"),
                spec=PatchedGenerationSpec(
                    engine=engine,
                    dataset=dataset,
                    patch=SwapMeanPatch(
                        centroids=StepRef("trade_size_centroids_aligned_patch"),
            ...
            WorkflowStep(
                name="swap_to_conflict_centroid_control",
                runner="gpu",
                depends_on=("baseline_conflict_rows", "trade_size_centroids_same_label_control"),
                spec=PatchedGenerationSpec(
                    engine=engine,
                    dataset=dataset,
                    patch=SwapMeanPatch(
                        centroids=StepRef("trade_size_centroids_same_label_control"),
            ...
            WorkflowStep(
                name="random_control_patch",
                runner="gpu",
                depends_on=("baseline_conflict_rows",),
                spec=PatchedGenerationSpec(
                    engine=engine,
                    dataset=dataset,
                    patch=RandomControlPatch(
                        subspace=StepRef("trade_size_subspace"),
                        write_site=ResidualInterventionSite(site="resid_post", layers=(WRITE_LAYER,)),
                        target_tokens=TokenSelector.last(),
                        strength=1.0,
                        random_seed=11,
            ...
            WorkflowStep(
                ...
                runner="cpu",
                depends_on=(
                    "swap_to_aligned_centroid",
                    "swap_to_conflict_centroid_control",
                    "random_control_patch",
                ),
                spec=PatchComparisonSpec(
                    baseline=StepRef("baseline_conflict_rows"),
                    variants={
                        "swap_to_aligned": StepRef("swap_to_aligned_centroid"),
                        "same_label_control": StepRef("swap_to_conflict_centroid_control"),
                        "random_control": StepRef("random_control_patch"),
                    },
