...
"""pipelines_v2 workflow scaffold for prompt_confusion phase_10."""
...
DEFAULT_REPORT_DIR = "projects/DX_TERMINAL/prompt_confusion/phase_10/reports/pipelines_v2"
CAPTURED_LAYERS = (0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44)


def _dataset_limit_from_env() -> int | None:
    raw = os.environ.get("PHASE_10_DATASET_LIMIT")
    if raw is None or not raw.strip():
        return None
    limit = int(raw)
    if limit <= 0:
        raise ValueError("PHASE_10_DATASET_LIMIT must be a positive integer")
    return limit


def _load_dataset_records(path: Path) -> list[dict[str, object]]:
    if not path.exists():
        raise FileNotFoundError(f"Phase 10 dataset not found: {path}")
    ...
    if not rows:
        raise ValueError(f"Phase 10 dataset is empty: {path}")
    return rows
...
def build_dataset(*, limit: int | None = None) -> Dataset:
    dataset = Dataset.from_records(
        ...
        name="prompt_confusion_phase_10_risk_preference",
    )
...
def build_runner_specs() -> dict[str, object]:
    artifact_store = ModalVolumeStore(
        name="xenon-data",
        root="/data/artifacts/prompt_confusion_phase_10",
    )
    return {
        ...
        "report_local": LocalRunnerSpec(
            artifacts=LocalArtifactStore(Path("artifacts") / "prompt_confusion_phase_10"),
            catalog=build_prompt_confusion_catalog(__file__),
        ),
    }
...
def build_workflow(
    ...
    *,
    ...
    return WorkflowSpec(
        name="prompt_confusion_phase_10_pipelines_v2",
        steps=(
            WorkflowStep(
