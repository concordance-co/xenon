...
APP_NAME = "xenon-prompt-confusion-phase11-behavior-smoke"
MODEL_ID = "Qwen/Qwen3-30B-A3B"
DEFAULT_INPUT = Path(
    "projects/DX_TERMINAL/prompt_confusion/phase_11/outputs/phase_11_dataset/phase_11_dataset.jsonl"
)
DEFAULT_OUTPUT = Path(
    "projects/DX_TERMINAL/prompt_confusion/phase_11/reports/behavior_smoke_double_conflict.json"
)
...
def _sample_rows(rows: list[dict[str, Any]], max_per_cell: int) -> list[dict[str, Any]]:
    ...
    by_cell: dict[tuple[str, str, str, int, int], list[dict[str, Any]]] = defaultdict(list)
    for row in rows:
        key = (
            str(row["strategy_size_direction"]),
            str(row["strategy_risk_direction"]),
            str(row["context_variant_id"]),
            int(row["size_setting_value"]),
            int(row["risk_setting_value"]),
        )
    ...
    for key, bucket in sorted(by_cell.items()):
        bucket_sorted = sorted(
            bucket,
            key=lambda r: (
                int(r["size_setting_value"]),
                int(r["risk_setting_value"]),
                str(r["example_id"]),
            ),
        )
...
def generate_rows(
    ...
    *,
    ...
    cfg = VLLMCaptureConfig(
        output_dir=Path("/tmp/phase11_behavior_smoke"),
...
def _summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        key = (
            f"{row['strategy_size_direction']}::{row['strategy_risk_direction']}::"
            f"{row['context_variant_id']}::size_{row['size_setting_value']}::risk_{row['risk_setting_value']}"
        )
        grouped[key].append(row)
    for key, bucket in sorted(grouped.items()):
        summary["by_cell"][key] = {
            "rows": len(bucket),
            "valid_output_rate": round(sum(bool(r["valid_output"]) for r in bucket) / len(bucket), 4),
            ...
            "size_counts": dict(Counter(str(r["size_label"]) for r in bucket)),
        }
    return summary
