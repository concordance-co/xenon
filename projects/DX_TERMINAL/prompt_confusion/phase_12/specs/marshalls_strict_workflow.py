...
"""Strict both-axes holdout battery for prompt_confusion phase_12."""
...
PHASE_ROOT = Path("projects/DX_TERMINAL/prompt_confusion/phase_12")
DEFAULT_DATASET_PATH = Path(
    os.environ.get(
        "PHASE_12_DATASET_PATH",
        str(PHASE_ROOT / "outputs" / "phase_12_dataset" / "phase_12_dataset.jsonl"),
    )
)
BATTERY_REPORT_DIR = str(PHASE_ROOT / "reports" / "marshalls_strict")
MODAL_ARTIFACT_ROOT = "/data/artifacts/prompt_confusion_phase_12_marshalls_strict"
LOCAL_ARTIFACT_ROOT = Path("artifacts") / "prompt_confusion_phase_12_marshalls_strict"
...
def _load_dataset_records() -> list[dict[str, object]]:
    if not DEFAULT_DATASET_PATH.exists():
        raise FileNotFoundError(f"Phase 12 dataset not found: {DEFAULT_DATASET_PATH}")
    ...
    if not rows:
        raise ValueError(f"Phase 12 dataset is empty: {DEFAULT_DATASET_PATH}")
    return rows
...
def build_dataset() -> Dataset:
    return Dataset.from_records(
        ...
        name="prompt_confusion_phase_12_marshalls_strict",
    )
...
def build_workflow(dataset: Dataset | None = None) -> WorkflowSpec:
    ...
    return WorkflowSpec(
        name="phase_12_marshalls_strict",
        steps=(
            WorkflowStep(
