...
A trading agent sees system rules, user strategies, active vault settings, portfolio state, market data, and prior decisions. Those sources are not always mutually consistent.

TODO: Add one concrete DX Terminal example in plain language: strategy says buy aggressively while settings cap size; strategy says sell all while settings imply partial exits; complaint refers to a strategy no longer active.
