...
PHASE_00_DOCS_ROOT = PROJECT_ARTIFACT_ROOT / "phase_00" / "docs"
PHASE_01_DOCS_ROOT = PROJECT_ARTIFACT_ROOT / "phase_01" / "docs"
...
def build_validation_notes_markdown(snapshot: dict[str, object]) -> str:
    return (
        frontmatter("00", ["projects/MOREBENCH/phase_00/outputs/benchmark_snapshot_detail.json"])
...
def build_labeling_functions_markdown(
    ...
    return (
        frontmatter(
            "01",
            [
                "projects/MOREBENCH/phase_01/outputs/prompt_side_labels.json",
                "projects/MOREBENCH/phase_01/outputs/response_side_labels.json",
            ],
        )
...
def main() -> None:
    ...
    write_text(
        PHASE_00_DOCS_ROOT / "00-validation-memo.md",
        frontmatter("00", ["projects/MOREBENCH/phase_00/outputs/benchmark_framing.json"])
        + "\n\n"
        + build_phase_00_report(benchmark_framing, confound_analysis, theory_pairing, action_locus_probeability)
        + "\n",
    )
    write_json(PHASE_00_DOCS_ROOT / "00-validation-summary.json", build_phase_00_summary_json(benchmark_framing))
    write_text(PHASE_00_DOCS_ROOT / "00-validation-notes.md", build_validation_notes_markdown(snapshot))

    write_text(
        PHASE_01_DOCS_ROOT / "01-latent-label-spec.md",
    ...
    write_csv(PHASE_01_DOCS_ROOT / "01-label-inventory.csv", native_label_inventory)
    write_text(
        PHASE_01_DOCS_ROOT / "01-labeling-functions.md",
        build_labeling_functions_markdown(prompt_side_labels, response_side_labels),
    )
    write_text(
        PHASE_01_DOCS_ROOT / "01-confound-audit.md",
        build_confound_audit_markdown(confound_analysis, action_locus_probeability),
    )
    write_csv(PHASE_01_DOCS_ROOT / "01-frozen-label-set.csv", frozen_label_rows)
    write_text(
        PHASE_01_DOCS_ROOT / "01-gap-list.md",
        build_gap_list_markdown(frozen_label_set_placeholder, follow_on_data_plan),
    )
    write_json(
        PHASE_01_DOCS_ROOT / "01-latent-label-summary.json",
        build_phase_01_summary_json(prompt_side_labels, response_side_labels, frozen_label_set_placeholder),
    )
    write_text(PHASE_01_DOCS_ROOT / "01-derivability-report.md", build_derivability_report_markdown())
