...
from projects.healthbench.shared.scope_vs_commit import build_scope_vs_commit_records


DEFAULT_TABLE = "healthbench_scope_vs_commit_smoke_v1"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Upload the small HealthBench scope-vs-commit smoke set into Neon."
    )
...
def ensure_table(conn: psycopg.Connection, table_name: str) -> None:
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS {table_name} (
            ...
            context_completeness_index INTEGER NOT NULL,
            axis TEXT NOT NULL,
            sample_index INTEGER NOT NULL,
            prompt_messages_json JSONB NOT NULL,
            prompt_sha256 TEXT NOT NULL,
            prompt_char_len INTEGER NOT NULL,
    ...
    conn.execute(
        ...
        f"ON {table_name} (context_condition)"
    )
    conn.execute(
        f"CREATE INDEX IF NOT EXISTS {table_name}_triple_condition_idx "
        f"ON {table_name} (triple_id, context_condition)"
    )
    conn.execute(
        f"CREATE INDEX IF NOT EXISTS {table_name}_prompt_hash_idx "
        f"ON {table_name} (prompt_sha256)"
    )
...
def main() -> None:
    ...
    rows = build_scope_vs_commit_records(
    ...
    print(f"Uploaded {len(rows)} HealthBench scope-vs-commit smoke rows to {table_name}.")
