...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build and upload the phase-6 profile invariance dataset.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("data/interp_exports/synthetic_market_phase6_profile_invariance"),
        help="Local directory for phase-6 profile invariance artifacts.",
    )
    parser.add_argument(
        "--permutation-variants",
        type=int,
        default=6,
        help="Number of row/symbol permutations to emit per scenario.",
    )
    parser.add_argument(
        "--profile-surface-variants",
        type=int,
        default=4,
        help="Number of row-surface prompt styles to emit per scenario.",
    )
    parser.add_argument(
        "--phase-name",
        default="phase6_profile_invariance",
...
def main() -> None:
    ...
    if args.log_id_base > 0:
        ...
        log_id_base = 2_146_940_000
    else:
        log_id_base = 2_146_950_000
