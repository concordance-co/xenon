...
case "$CMD" in
  download)
    ...
    uv run --extra interp --extra modal modal run pipelines/interp/modal_vllm_orchestrator.py::download_model "$@"
    ;;
  smoke)
    echo "Smoke test: 1 example, layer 24..."
    uv run --extra interp --extra modal modal run pipelines/interp/modal_vllm_orchestrator.py --limit 1 --layers 24 "$@"
    ;;
  router)
    echo "Router logits only (detached, no residual)..."
    uv run --extra interp --extra modal modal run --detach pipelines/interp/modal_vllm_orchestrator.py --no-capture-residual "$@"
    ;;
  full)
    echo "Full capture (detached, residual + router)..."
    uv run --extra interp --extra modal modal run --detach pipelines/interp/modal_vllm_orchestrator.py "$@"
    ;;
  inspect)
    echo "Inspecting volume contents..."
    uv run --extra interp --extra modal modal run pipelines/interp/modal_vllm_orchestrator.py::inspect_volume "$@"
    ;;
  meta)
    echo "Local metadata:"
...
"
  ...
  modal-ingest)
    ...
    uv run --extra interp --extra modal modal run --detach pipelines/ingest/modal.py --mode ingest "$@"
    ;;
  modal-backfill)
    echo "Backfilling missing full logs and null payloads (detached)..."
    uv run --extra interp --extra modal modal run --detach pipelines/ingest/modal.py --mode ingest --selection backfill "$@"
    ;;
  modal-prep)
    echo "Running data prep on Modal (detached, reads/writes Neon Postgres)..."
    uv run --extra interp --extra modal modal run --detach pipelines/ingest/modal.py --mode prep "$@"
    ;;
  manifold-export)
    echo "Exporting market manifold tables from Neon full_logs.raw_payload..."
    uv run python -m pipelines.ingest.manifold_export "$@"
    ;;
  modal-outcomes)
    echo "Computing trade outcomes on Modal (detached)..."
    uv run --extra interp --extra modal modal run --detach pipelines/ingest/modal.py --mode outcomes "$@"
    ;;
  migrate-to-neon)
  ...
  reset-cursors)
    ...
    uv run --extra interp --extra modal modal run pipelines/ingest/modal.py --mode reset-cursors "$@"
    ;;
  counterfactual-build)
    echo "Building counterfactual datasets (local → Neon DB)..."
    uv run python -m research.counterfactual "$@"
    ;;
  counterfactual-capture)
    echo "Running counterfactual capture on Modal (vLLM, H200, detached)..."
    uv run --extra interp --extra modal modal run --detach pipelines/interp/modal_vllm_orchestrator.py --mode counterfactual "$@"
    ;;
  ...
  counterfactual-structure)
    ...
    ;;
  *)
    echo "Usage: $0 {download|smoke|router|full|inspect|meta|compact|analyze-local|analyze|decision-structure-pool|decision-structure-analyze|modal-ingest|modal-backfill|modal-prep|manifold-export|modal-outcomes|download-activations|download-results|counterfactual-build|counterfactual-capture|counterfactual-analyze|counterfactual-structure} [extra flags]"
    echo ""
