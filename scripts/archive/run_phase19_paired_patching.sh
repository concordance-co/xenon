...
run_one() {
  ...
  uv run --extra interp --extra modal modal run \
    pipelines/interp/patching/modal.py::run_synthetic_market_behavior_modal \
    "$@" >"$log_path" 2>&1
  printf 'finished %s\n' "$run_name"
}
...
run_donor_prep() {
  ...
  uv run --extra interp --extra modal modal run \
    pipelines/interp/patching/modal.py::prepare_synthetic_market_behavior_donors_modal \
    "$@" >"$log_path" 2>&1
