...
launch_run() {
  ...
  if [[ "$LAUNCH_MODE" == "print" ]]; then
    printf 'modal run pipelines/interp/patching/modal.py::run_synthetic_market_behavior_matrix_modal'
    printf ' %q' "$@"
    printf '\n'
    return
  fi
  if [[ "$LAUNCH_MODE" == "sequential" ]]; then
    modal run pipelines/interp/patching/modal.py::run_synthetic_market_behavior_matrix_modal \
      "$@" >"$log_path" 2>&1
    printf 'finished %s\n' "$run_name"
    return
  fi
  nohup modal run pipelines/interp/patching/modal.py::run_synthetic_market_behavior_matrix_modal \
    "$@" >"$log_path" 2>&1 &
