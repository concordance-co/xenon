...
run_donor_prep() {
  ...
  modal run pipelines/interp/patching/modal.py::prepare_synthetic_market_behavior_donors_modal \
...
run_generation() {
  ...
  modal run pipelines/interp/patching/modal.py::run_synthetic_market_behavior_modal \
    --phase-name "$PHASE_NAME" \
...
launch_generation_background() {
  ...
  nohup modal run pipelines/interp/patching/modal.py::run_synthetic_market_behavior_modal \
    --phase-name "$PHASE_NAME" \
    --run-name "$GEN_RUN_NAME" \
    --model-id "$MODEL_ID" \
    --context-variant "$CONTEXT_VARIANT" \
