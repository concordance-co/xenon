...
Usage (from repo root):
    uv run python scripts/db/neon_query.py tables              # list all tables + row counts
    uv run python scripts/db/neon_query.py schema              # all table schemas
    uv run python scripts/db/neon_query.py schema vaults       # one table's schema
    uv run python scripts/db/neon_query.py sample vaults       # 5 sample rows (default)
    uv run python scripts/db/neon_query.py sample vaults 20    # 20 sample rows
    uv run python scripts/db/neon_query.py query "SELECT ..."  # run arbitrary read-only SQL
    uv run python scripts/db/neon_query.py query -f query.sql  # run SQL from a file
