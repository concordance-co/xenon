...
RESULTS_PATH = Path("data/analysis_results/synthetic_market_representation/phase7_relation_invariance_v1/results.json")
ASSET_EXPORT_PATH = Path("data/interp_exports/synthetic_market_phase7_relation_invariance/synthetic_market_asset_records.parquet")
OUTPUT_DIR = Path("data/report_assets/synthetic_market_phase7_relation_invariance")
...
SCENARIO_LABELS = {
    "momentum_edge_near_tie": "Momentum Edge",
    "flow_edge_near_tie": "Flow Edge",
    "broad_participation_edge": "Broad Participation",
    "concentration_penalty_edge": "Concentration Penalty",
}
...
def rank_bucket_chart() -> Path:
    ...
    ax.set_title("Phase 7 explicitly moves the anchor pair across different roster-rank contexts", loc="left", fontsize=14, fontweight="bold", color=CHARCOAL)
...
def relation_modes_chart(data: dict) -> Path:
    ...
    axes[0].set_title("Anchor-pair relations are strongest under style/layout, then roster and magnitude controls", loc="left", fontsize=13, fontweight="bold", color=CHARCOAL)
    ...
    axes[1].set_title("Relation retrieval accuracy stays high across the new control axes", loc="left", fontsize=13, fontweight="bold", color=CHARCOAL)
...
def control_chart(data: dict) -> Path:
    ...
    axes[0].set_title("Phase 7 asks whether relation identity beats rank bucket and scale bucket", loc="left", fontsize=13, fontweight="bold", color=CHARCOAL)
    axes[0].legend(frameon=False, ncol=2, loc="upper right")
    ...
    axes[1].set_ylim(0.0, 1.05)
    axes[1].set_title("The comparison is not just invariance, but what the relation vector prefers to group by", loc="left", fontsize=13, fontweight="bold", color=CHARCOAL)
