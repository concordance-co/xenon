...
REPRESENTATION_RESULTS_PATH = Path(
    "data/analysis_results/synthetic_market_representation/phase14_set_geometry_affordance_ladder_v1"
)
TRANSFORM_RESULTS_PATH = Path(
    "data/analysis_results/synthetic_market_transform/phase14_set_geometry_affordance_ladder_v1/phase14_explicit_transforms_v1"
)
EXPORT_ASSETS_PATH = Path(
    "data/interp_exports/synthetic_market_phase14_set_geometry_affordance_ladder/synthetic_market_asset_records.parquet"
)
SUMMARY_PATH = Path(
    "data/interp_exports/synthetic_market_phase14_set_geometry_affordance_ladder/synthetic_market_summary.json"
)
OUTPUT_DIR = Path("data/report_assets/synthetic_market_phase14_affordance_ladder")
...
GRID = "#D6DEE3"
CHARCOAL = "#21313F"
AFFORDANCE_COLORS = ["#16324F", "#275D73", "#377A7D", "#6A8F4E", "#A37A39", "#B56662"]
ASSET_COLORS = {
    "geo_alpha": NAVY,
    "geo_beta": TEAL,
    "geo_gamma": GOLD,
...
def experiment_design_chart(summary: dict) -> Path:
    ...
    ax_data.text(
        0.0,
        0.82,
        "The dataset repeats the same\n4-asset markets under progressively\nharder execution constraints.",
        fontsize=9.5,
        color=SLATE,
    ...
    levels = np.arange(6)
    labels = ["Market", "A1", "A2", "A3", "A4", "A5"]
    unrestricted = np.array([4.0, 3.6, 2.7, 2.0, 1.3, 1.0])
    blocked_pressure = np.array([0.0, 1.2, 2.0, 3.0, 3.7, 4.4])
    ax_ladder.bar(levels - 0.18, unrestricted, width=0.36, color=TEAL, label="Effective unrestricted routes")
    ax_ladder.bar(levels + 0.18, blocked_pressure, width=0.36, color=ROSE, label="Execution pressure")
    for x, open_routes, blocked in zip(levels, unrestricted, blocked_pressure, strict=True):
        ax_ladder.text(x - 0.18, open_routes + 0.08, f"{open_routes:.1f}", ha="center", va="bottom", fontsize=8, color=CHARCOAL)
        ax_ladder.text(x + 0.18, blocked + 0.08, f"{blocked:.1f}", ha="center", va="bottom", fontsize=8, color=CHARCOAL)
    ax_ladder.set_xticks(levels, labels)
    ax_ladder.set_ylim(0, 5.2)
    ax_ladder.set_ylabel("Constraint intensity")
    ax_ladder.set_title(
...
def coordinate_transfer_chart(data: dict) -> Path:
    ...
    ax.set_ylim(0.96, 1.0005)
    ax.set_xticks(x, [_context_label(context) for context in contexts])
    ax.set_ylabel("Held-out R²")
    ax.set_title(
        "The shared 4-asset market axes survive the full affordance ladder",
        loc="left",
        fontsize=14,
        fontweight="bold",
        color=CHARCOAL,
...
def context_realignment_chart(data: dict) -> Path:
    ...
    for ax, context, color in zip(axes_arr.flat, contexts, AFFORDANCE_COLORS, strict=False):
...
def context_deformation_chart(data: dict) -> Path:
    ...
    axes[0].bar(x, spearman, color=AFFORDANCE_COLORS[: len(pairs)], edgecolor="none")
    _setup_axes(axes[0])
    ...
    axes[0].set_title(
        "Adjacent affordance steps induce structured geometry changes",
        loc="left",
        fontsize=13,
        fontweight="bold",
        color=CHARCOAL,
    ...
    axes[1].bar(x, cosine, color=AFFORDANCE_COLORS[: len(pairs)], edgecolor="none")
    _setup_axes(axes[1])
...
def representative_score_geometry_chart() -> Path:
    ...
    fig.suptitle(
        "Representative score geometry across the affordance ladder",
        x=0.02,
        y=1.01,
        ha="left",
        fontsize=14,
...
def family_heatmap_chart(data: dict) -> Path:
    ...
    for ax, state_name in zip(axes, ["early", "late"], strict=True):
        ...
        vmin = 0.55 if state_name == "late" else 0.97
...
def composition_chart(data: dict) -> Path:
    ...
    for ax, state_name in zip(axes, ["early", "late"], strict=True):
        ...
        ax.set_ylim(0.4 if state_name == "late" else 0.95, 1.01)
        ax.set_title(
...
def main() -> None:
    ...
    print(f"Wrote Phase 14 chart assets to {OUTPUT_DIR}")
