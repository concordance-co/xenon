...
def _refusal_dataset() -> Dataset:
    metadata = _token_metadata("instruction", "completion")
...
def _truthfulness_dataset() -> Dataset:
    metadata = _token_metadata("question", "answer")
    return Dataset.from_examples(
        [
...
def _token_metadata(first: str, second: str) -> dict[str, object]:
    return {
        "token_sections": {
            first: [0, 1, 2],
            second: [3, 4, 5],
        ...
        "section_records": [
            {"name": first, "token_positions": [0, 1, 2], "index": 0},
            {"name": second, "token_positions": [3, 4, 5], "index": 1},
        ],
    }
